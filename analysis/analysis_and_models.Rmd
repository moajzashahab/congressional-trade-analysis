...
```
```{r}
X_train = as.matrix(train_data[-trade_success])
...
```{r, message=FALSE, warning=FALSE}
X_train_trunc <- X_train[,-c(103:254)]
X_test_trunc <- X_test[, -c(103:254)]
```
...
```{r, message=FALSE, warning=FALSE}
predictions = predict(model, X_test_trunc, s = best_lambda, type = "response")
predicted_classes = ifelse(predictions > 0.5, 1, 0)
...
print(significant_coefficients)
```
#Doing Ridge, but with only committees as features
```{r, message=FALSE, warning=FALSE}
...
```{r, message=FALSE, warning=FALSE}
predictions = predict(model, X_test_select, s = best_lambda, type = "response")
predicted_classes = ifelse(predictions > 0.5, 1, 0)
